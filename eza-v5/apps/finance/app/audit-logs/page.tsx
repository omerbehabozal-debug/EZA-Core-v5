 ...
 * 
 * Event-level inspection only for finance-related activity.
...
import { apiClient, FinanceAuditLogsResponse } from '@/lib/api-client';
import { useFinanceAuth } from '@/lib/auth-guard';

export default function AuditLogsPage() {
  const { isAuthorized, loading } = useFinanceAuth();
  const [auditLogs, setAuditLogs] = useState<FinanceAuditLogsResponse | null>(null);
  ...
  useEffect(() => {
    ...
    const fetchAuditLogs = async () => {
      try {
        ...
        const response = await apiClient.get<FinanceAuditLogsResponse>(
          `/api/proxy/finance/audit-logs?${params.toString()}`
        );
          ...
          <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
            ...
            <div>
              ...
              >
                ...
                <option value="Misleading advice">Yanıltıcı Danışmanlık</option>
                <option value="Bias in scoring">Önyargılı Skorlama</option>
                <option value="Manipulative framing">Manipülatif Çerçeveleme</option>
                <option value="Overconfidence risk">Aşırı Güven Riski</option>
              </select>
            </div>
            <div>
