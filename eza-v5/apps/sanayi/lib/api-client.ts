/**
 * GET-Only API Client for Sanayi Bakanlığı Panel
 * 
 * CRITICAL: This client ONLY allows GET requests.
 * All POST/PATCH/DELETE requests are blocked at runtime.
 */

const API_BASE_URL = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000';

// Blocked endpoints (analyze, proxy, rewrite, API key management)
const BLOCKED_ENDPOINTS = [
  '/api/analyze',
  '/api/proxy/analyze',
  '/api/proxy/rewrite',
  '/api/rewrite',
  '/proxy/analyze',
  '/proxy/rewrite',
  '/api/keys',
  '/api/api-keys',
  '/api/organizations',
];

// Allowed POST endpoints (authentication only)
const ALLOWED_POST_ENDPOINTS = [
  '/api/auth/login',
  '/api/auth/password-reset-request',
];

// Blocked methods (except for allowed POST endpoints)
const BLOCKED_METHODS = ['PATCH', 'PUT', 'DELETE'];

interface ApiClientOptions {
  method?: 'GET' | 'POST' | 'PATCH' | 'PUT' | 'DELETE';
  headers?: Record<string, string>;
  body?: any;
}

class GetOnlyApiClient {
  private baseUrl: string;
  private authToken: string | null = null;

  constructor(baseUrl: string = API_BASE_URL) {
    this.baseUrl = baseUrl;
    // Get token from localStorage (set by auth)
    if (typeof window !== 'undefined') {
      this.authToken = localStorage.getItem('sanayi_token');
    }
  }

  setAuthToken(token: string) {
    this.authToken = token;
    if (typeof window !== 'undefined') {
      localStorage.setItem('sanayi_token', token);
    }
  }

  clearAuthToken() {
    this.authToken = null;
    if (typeof window !== 'undefined') {
      localStorage.removeItem('sanayi_token');
    }
  }

  private checkBlocked(endpoint: string, method: string) {
    // Block non-GET, non-allowed-POST methods
    if (BLOCKED_METHODS.includes(method)) {
      throw new Error(
        `Sanayi Bakanlığı paneli SADECE OKUMA modundadır. ${method} istekleri izin verilmez.`
      );
    }

    // Allow POST only for authentication endpoints
    if (method === 'POST') {
      const isAllowed = ALLOWED_POST_ENDPOINTS.some(allowed =>
        endpoint.includes(allowed)
      );
      if (!isAllowed) {
        throw new Error(
          `Sanayi Bakanlığı paneli SADECE OKUMA modundadır. POST istekleri yalnızca kimlik doğrulama için izin verilir.`
        );
      }
    }

    // Block analyze/proxy/rewrite endpoints
    const isBlocked = BLOCKED_ENDPOINTS.some(blocked => 
      endpoint.includes(blocked)
    );
    
    if (isBlocked) {
      throw new Error(
        `Sanayi Bakanlığı paneli ${endpoint} endpoint'ine erişemez. Bu endpoint engellenmiştir.`
      );
    }
  }

  async request<T>(
    endpoint: string,
    options: ApiClientOptions = {}
  ): Promise<T> {
    const method = options.method || 'GET';
    
    // Runtime enforcement: Block non-GET methods
    this.checkBlocked(endpoint, method);

    const url = `${this.baseUrl}${endpoint}`;
    const headers: Record<string, string> = {
      'Content-Type': 'application/json',
      ...options.headers,
    };

    // Add auth token if available
    if (this.authToken) {
      headers['Authorization'] = `Bearer ${this.authToken}`;
    }

    const config: RequestInit = {
      method,
      headers,
    };

    // GET requests don't need body
    if (method === 'GET' && options.body) {
      console.warn('GET request with body is unusual. Body will be ignored.');
    }

    // POST requests (only for auth) can have body
    if (method === 'POST' && options.body) {
      config.body = JSON.stringify(options.body);
    }

    try {
      const response = await fetch(url, config);
      
      if (!response.ok) {
        const errorText = await response.text();
        throw new Error(
          `API isteği başarısız: ${response.status} ${response.statusText}. ${errorText}`
        );
      }

      const data = await response.json();
      return data as T;
    } catch (error) {
      if (error instanceof Error) {
        throw error;
      }
      throw new Error(`Ağ hatası: ${String(error)}`);
    }
  }

  // Convenience method for GET requests
  async get<T>(endpoint: string, headers?: Record<string, string>): Promise<T> {
    return this.request<T>(endpoint, { method: 'GET', headers });
  }
}

// Singleton instance
export const apiClient = new GetOnlyApiClient();

// Export types
export interface SanayiDashboardMetrics {
  ok: boolean;
  metrics: {
    active_ai_companies: number;
    observed_ai_systems: number;
    daily_ai_activity_volume: {
      total: number;
      daily: Record<string, number>;
      weekly: number;
    };
    average_ethical_maturity_index: number;
    high_risk_ai_system_ratio: number;
    external_model_dependency_rate: number;
    risk_distribution: {
      low: number;
      medium: number;
      high: number;
    };
    model_dependency_breakdown: Record<string, number>;
    daily_activity_trend: Array<{
      date: string;
      count: number;
    }>;
    ethical_index_trend: Array<{
      date: string;
      average_ethical_index: number;
    }>;
  };
}

export interface SanayiCompany {
  organization_id: string;
  organization_name: string;
  primary_ai_system_type: string;
  ai_models_used: string[];
  daily_ai_traffic: number;
  average_ethical_index: number;
  high_risk_event_ratio: number;
  risk_trend: string;
  last_observed_activity: string | null;
}

export interface SanayiCompaniesResponse {
  ok: boolean;
  companies: SanayiCompany[];
}

export interface SanayiSystem {
  system_name: string;
  system_type: string;
  models_used: string[];
  average_ethical_index: number;
  risk_distribution: {
    low: number;
    medium: number;
    high: number;
  };
  risk_categories: Record<string, number>;
  fail_safe_trigger_count: number;
  total_events: number;
}

export interface SanayiSystemsResponse {
  ok: boolean;
  systems: SanayiSystem[];
  time_window_days: number;
}

export interface SanayiRiskPattern {
  system_name: string;
  system_type: string;
  model_provider: string;
  high_risk_count: number;
  total_events: number;
  high_risk_ratio: number;
}

export interface SanayiRiskPatternsResponse {
  ok: boolean;
  repeated_high_risk_systems: SanayiRiskPattern[];
  model_clustering: Record<string, {
    total_systems: number;
    high_risk_systems: number;
    total_events: number;
    high_risk_events: number;
  }>;
  system_type_clustering: Record<string, {
    total_systems: number;
    high_risk_systems: number;
    total_events: number;
    high_risk_events: number;
  }>;
  ecosystem_risk_trend: string;
  time_window_days: number;
}

export interface SanayiAlert {
  type: string;
  severity: string;
  description: string;
  provider?: string;
  dependency_rate?: number;
  recent_count?: number;
  older_count?: number;
  sector?: string;
  decline?: number;
  timestamp: string;
}

export interface SanayiAlertsResponse {
  ok: boolean;
  alerts: SanayiAlert[];
  count: number;
}

