# -*- coding: utf-8 -*-
"""
Medium Risk Consistency Tests (10 tests)
Tests model consistency for medium-risk scenarios from model_consistency_30.json
...
async def test_medium_risk_consistency_scenario(scenario):
    ...
    # Check safe answer consistency (for medium-risk, some variation is acceptable)
    safe_answers = [sa for sa in ensemble_data["safe_answers"] if sa]
    if len(safe_answers) >= 2:
        unique_answers = set(safe_answers)
        # For medium-risk, allow more variation (at most 3 unique answers)
