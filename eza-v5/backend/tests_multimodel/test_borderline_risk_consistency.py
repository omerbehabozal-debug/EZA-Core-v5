...
async def test_borderline_risk_consistency_scenario(scenario):
    ...
    # Check score deviation (borderline scenarios may have higher deviation) - only if 2+ models
    max_score = max(scores)
    min_score = min(scores)
    score_deviation = max_score - min_score
    
    # Allow 300% tolerance for score deviation (models can vary significantly, especially for borderline)
    ...
    if len(alignment_scores) > 1:
        ...
        # Allow 300% tolerance for alignment diff (models can vary significantly, especially for borderline)
