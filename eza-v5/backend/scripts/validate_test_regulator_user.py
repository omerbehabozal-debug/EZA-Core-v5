...
async def validate_test_regulator_user():
    ...
    async with async_session() as session:
        try:
            ...
            if not user:
                print(f"❌ Test regulator user not found: {TEST_USER_EMAIL}")
            ...
            if user.role == "REGULATOR_AUDITOR":
                ...
                print(f"❌ Role: {user.role} (expected: REGULATOR_AUDITOR)")
                checks.append(False)
            ...
            if is_test:
                print(f"✓ Is Internal Test User: {is_test}")
                checks.append(True)
            else:
                print(f"⚠ Is Internal Test User: {is_test} (expected: True)")
                checks.append(False)
            
            # Check no organization
            org_users = user.organization_users if hasattr(user, 'organization_users') else []
            if len(org_users) == 0:
                print(f"✓ No Organization: User has no organization_id (correct)")
                checks.append(True)
            else:
                print(f"❌ Organization: User has {len(org_users)} organization(s) (expected: 0)")
                checks.append(False)
            
            # Check no API keys
            api_keys = user.api_keys if hasattr(user, 'api_keys') else []
            if len(api_keys) == 0:
                print(f"✓ No API Keys: User has no API keys (correct)")
                checks.append(True)
            else:
                print(f"⚠ API Keys: User has {len(api_keys)} API key(s) (expected: 0)")
                checks.append(False)
            ...
            if all(checks):
                ...
                print(f"   ❌ Cannot access /proxy (403 Forbidden)")
                print(f"   ❌ Cannot access /platform (403 Forbidden)")
                print(f"   ❌ Cannot access /admin (403 Forbidden)")
                print(f"   ✅ All API calls are GET-only")
                return True
            else:
                print("❌ Some validations failed. Please check the user properties.")
                return False
                
        except Exception as e:
            print(f"❌ Error validating test regulator user: {e}")
            import traceback
            traceback.print_exc()
            return False
        finally:
