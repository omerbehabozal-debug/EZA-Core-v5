...
def _is_discoverable_root(node: MirrorNetworkNode) -> bool:
    ...
    if not evaluate_mirror_network_safety(node).passed:
        return False
    scene = ensure_persistable_mirror_scene_url(node.scene_image_url)
    return bool(scene)
...
def _batch_yansi_counts(
    ...
    for child in children:
        if not is_eligible_yansi_child(child):
            continue
...
async def _fetch_children_for_parents(
    ...
    result = await db.execute(
        select(MirrorNetworkNode).where(MirrorNetworkNode.parent_slug.in_(normalized))
    )
...
async def list_discover_mirrors(
    ...
    *,
    ...
    candidates = [node for node in result.scalars().all() if _is_discoverable_root(node)]

    slugs = [node.slug for node in candidates]
    children = await _fetch_children_for_parents(db, slugs)
    yansi_by_parent = _batch_yansi_counts(children)

    def sort_key(node: MirrorNetworkNode) -> tuple[int, float]:
    ...
    items = [
        _to_discover_item(node, yansi_count=yansi_by_parent.get(node.slug.lower(), 0))
        for node in page
    ]

    payload = DiscoverMirrorListResponse(items=items, total=len(candidates))
